...
resource "google_compute_global_address" "this" {
  ...
  name          = length("${var.network_name}-db") >= 63 ? "${substr(var.network_name, 0, 60)}-db" : "${var.network_name}-db"
  purpose       = "VPC_PEERING"
...
resource "google_vpc_access_connector" "this" {
  ...
  name          = length("${var.network_name}-connector") >= 25 ? "${substr(var.network_name, 0, 15)}-connector" : "${var.network_name}-connector"
